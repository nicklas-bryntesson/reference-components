...
| `--_toggletip-border-radius` | `var(--ui-radius)` | Bubble corner radius |
...
| `--_toggletip-border-color` | `var(--ui-border)` | Bubble border and title divider colour |
...
No CSS kernel dependency. The site tokens this component reads (`--SITE--PADDING`, `--ui-border`, `--ui-radius`, `--ui-shadow`) are surfaced through the `--_toggletip-*` CSS Variable API above. Most have fallbacks, so the component largely runs standalone — with two exceptions: `box-shadow: var(--ui-shadow)` has no fallback (no shadow without the token), and `--_toggletip-border-color`'s fallback chain bottoms out in `--ui-border` (the border degrades to `currentColor` without it).
