...
class MonthField {
  ...
  private _handlePopupKeydown(e: KeyboardEvent): void {
    if (e.key === 'Tab') {
      ...
      if (!this.popupEl) return
      const allCols = [...this.popupEl.querySelectorAll<HTMLElement>('.Wheel[role="spinbutton"]')]
      if (col && allCols.includes(col)) {
        ...
        if (!e.shiftKey && isLastCol) {
          e.preventDefault()
          this.popupEl.querySelector<HTMLButtonElement>('.MonthField-popup-clear')?.focus()
