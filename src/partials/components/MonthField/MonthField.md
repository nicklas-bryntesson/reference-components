...
|---|---|---|---|
| `native` | `<input type="month">` | The real form control; hidden in `custom` mode, the transparent tap layer in `display` mode | suite + JS |
...
| `segment` | `<span role="spinbutton">` | One editable segment; `data-segment` says which (`month` · `year`) | suite + JS |
...
| `year-month-picker` | `<div class="WheelColumns">` | Row of wheel hosts (`.Wheel[data-picker]`) | suite |
| `footer` | `<div>` | Popup footer holding the two actions | suite |
| `footer-clear` · `footer-now` | `<button>` | Clear / This month — both commit and close | suite + JS |
