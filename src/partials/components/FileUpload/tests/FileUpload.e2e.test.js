...
test('native input is aria-hidden', async ({ page }) => {
  const input = page.locator('[data-component="FileUpload"][data-initialized="true"] [data-part="input"]').last()
...
test('trigger button has accessible name', async ({ page }) => {
  const trigger = page.locator('[data-component="FileUpload"][data-initialized="true"] [data-part="trigger"]').last()
...
test('selecting a file adds it to the list', async ({ page }) => {
  ...
  const input = liveRoot.locator('[data-part="input"]')
  await input.setInputFiles(tmpFile)

  await expect(liveRoot.locator('[data-part="item"]')).toHaveCount(1)
...
test('remove button removes the file from the list', async ({ page }) => {
  ...
  await liveRoot.locator('[data-part="item-remove"]').click()

  await expect(liveRoot.locator('[data-part="item"]')).toHaveCount(0)
...
test('focus moves to trigger after removing the only file', async ({ page }) => {
  ...
  const removeBtn = liveRoot.locator('[data-part="item-remove"]')
  await removeBtn.focus()
  await removeBtn.click()

  await expect(liveRoot.locator('[data-part="trigger"]')).toBeFocused()
})
...
test('trigger is reachable via Tab', async ({ page }) => {
  const liveRoot = page.locator('[data-component="FileUpload"][data-initialized="true"]').last()
  const trigger = liveRoot.locator('[data-part="trigger"]')
  await trigger.focus()
...
test('file not matching accept shows invalid-type error', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    const inputEl = liveEl.querySelector('[data-part="input"]')
  ...
  await liveRoot.locator('[data-part="input"]').setInputFiles({
    name: 'document.txt',
    mimeType: 'text/plain',
    buffer: Buffer.from('hello'),
  })

  const invalidItem = liveRoot.locator('[data-part="item"][data-status="invalid-type"]')
  await expect(invalidItem).toHaveCount(1)
  await expect(invalidItem.locator('[data-part="item-error"]')).toBeVisible()
  await expect(invalidItem.locator('[data-part="item-error"]')).toHaveAttribute('role', 'alert')
})
...
test('file exceeding max size shows invalid-size error', async ({ page }) => {
  ...
  const invalidItem = liveRoot.locator('[data-part="item"][data-status="invalid-size"]')
  await expect(invalidItem).toHaveCount(1)
  await expect(invalidItem.locator('[data-part="item-error"]')).toBeVisible()
  await expect(invalidItem.locator('[data-part="item-error"]')).toHaveAttribute('role', 'alert')
})
...
test('file list has aria-live="polite" and aria-relevant="additions removals"', async ({ page }) => {
  const liveRoot = page.locator('[data-component="FileUpload"][data-initialized="true"]').last()
  const list = liveRoot.locator('[data-part="list"]')
...
test('remove button has aria-label containing the filename', async ({ page }) => {
  const tmpFile = path.join(__dir, 'fixtures', 'test.pdf')
  const liveRoot = page.locator('[data-component="FileUpload"][data-initialized="true"]').last()
  await liveRoot.locator('[data-part="input"]').setInputFiles(tmpFile)
...
test('error alert does not steal focus when invalid file is added', async ({ page }) => {
  ...
  await liveRoot.locator('[data-part="trigger"]').focus()

  await page.evaluate(() => {
    const el = Array.from(document.querySelectorAll('[data-component="FileUpload"][data-initialized="true"]')).at(-1)
    el.querySelector('[data-part="input"]').setAttribute('accept', '.pdf')
  })
  ...
  await expect(liveRoot.locator('[data-part="item-error"][role="alert"]')).toBeVisible()
...
test('focus moves to next sibling remove button when removing first of multiple files', async ({ page }) => {
  ...
  await page.evaluate(() => {
    const el = Array.from(document.querySelectorAll('[data-component="FileUpload"][data-initialized="true"]')).at(-1)
    el.querySelector('[data-part="input"]').setAttribute('multiple', '')
  })
  ...
  await expect(liveRoot.locator('[data-part="item"]')).toHaveCount(2)

  // Focus and remove the first item
  const firstRemove = liveRoot.locator('[data-part="item-remove"]').first()
  await firstRemove.focus()
  await firstRemove.click()

  // Focus should move to the remaining item's remove button
  await expect(liveRoot.locator('[data-part="item-remove"]')).toBeFocused()
})
...
test('drop-zone remove button removes the file instead of reopening the picker', async ({ page }) => {
  ...
  await dropZone.locator('[data-part="input"]').setInputFiles({
    name: 'dropped.pdf',
    mimeType: 'application/pdf',
    buffer: Buffer.from('x'),
  })
  await expect(dropZone.locator('[data-part="item-name"]')).toHaveText('dropped.pdf')
  ...
  await dropZone.locator('[data-part="item-remove"]').click()

  await expect(dropZone.locator('[data-part="item-name"]')).toHaveCount(0)
...
test('server files state has data-source=server and hidden input', async ({ page }) => {
  ...
  const serverItem = serverRoot.locator('[data-part="item"][data-source="server"]')
