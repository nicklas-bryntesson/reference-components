...
test('custom overlay is shown on pointer:fine', async ({ page }) => {
  await expect(page.locator(`${WF} [data-part="overlay"]`)).toBeVisible()
})
...
test('week segment has required aria attributes', async ({ page }) => {
  const week = page.locator(`${WF} [data-part="segment"][data-segment="week"]`)
...
test('year segment has role=spinbutton', async ({ page }) => {
  const year = page.locator(`${WF} [data-part="segment"][data-segment="year"]`)
...
test('segments group has role=group', async ({ page }) => {
  await expect(page.locator(`${WF} [data-part="segments"]`)).toHaveAttribute('role', 'group')
})

// ── Trigger ARIA ───────────────────────────────────────────────────────────────

test('trigger has aria-expanded=false and aria-haspopup=dialog when closed', async ({ page }) => {
  const trigger = page.locator(`${WF} [data-part="trigger"]`)
...
test('popup does not exist in DOM when closed', async ({ page }) => {
  await expect(page.locator(`${WF} [data-part="popup"]`)).toHaveCount(0)
})

test('popup is visible after trigger click', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  await expect(page.locator(`${WF} [data-part="popup"]`)).toBeVisible()
})

test('trigger aria-expanded=true when popup open', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  await expect(page.locator(`${WF} [data-part="trigger"]`)).toHaveAttribute('aria-expanded', 'true')
})
...
test('popup closes on Escape and focus returns to trigger', async ({ page }) => {
  ...
  await expect(page.locator(`${WF} [data-part="popup"]`)).toHaveCount(0)
  await expect(page.locator(`${WF} [data-part="trigger"]`)).toBeFocused()
})
...
test('popup has role=dialog and aria-modal=true', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  const popup = page.locator(`${WF} [data-part="popup"]`)
...
test('popup has localized aria-label', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  await expect(page.locator(`${WF} [data-part="popup"]`)).toHaveAttribute('aria-label', 'Choose week')
})

test('week grid has role=grid with a week-number column', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  const grid = page.locator(`${WF} [data-part="calendar-grid"]`)
  await expect(grid).toHaveAttribute('role', 'grid')
  // First column header + a week-number cell in the first row.
  await expect(page.locator(`${WF} [data-part="calendar-grid"] thead th[data-part="week-number-head"]`)).toBeVisible()
  await expect(page.locator(`${WF} [data-part="calendar-grid"] tbody tr`).first().locator('td[data-part="week-number-cell"]')).toHaveCount(1)
})

test('week rows are selectable rows with an aria-label naming the week', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  const firstRow = page.locator(`${WF} [data-part="calendar-grid"] tbody tr`).first()
...
test('clicking a week row selects that whole week and closes', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  const firstRow = page.locator(`${WF} [data-part="calendar-grid"] tbody tr:not([data-disabled="true"])`).first()
  const iso = await firstRow.getAttribute('data-week')
  await firstRow.click()
  await expect(page.locator(`${WF} [data-part="popup"]`)).toHaveCount(0)
  await expect(page.locator(`${WF} [data-part="native"]`)).toHaveValue(iso)
})
...
test('clicking a day cell in a row selects the whole week (row highlight)', async ({ page }) => {
  ...
  const row = page.locator(`${WF} [data-part="calendar-grid"] tbody tr:not([data-disabled="true"])`).nth(1)
...
test('ArrowDown moves the week-row highlight (roving tabindex on the row)', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  const focusedRow = page.locator(`${WF} [data-part="calendar-grid"] tbody tr[tabindex="0"]`)
  const startISO = await focusedRow.getAttribute('data-week')
  await page.keyboard.press('ArrowDown')
  const newFocused = page.locator(`${WF} [data-part="calendar-grid"] tbody tr[tabindex="0"]`)
...
test('selected week row is highlighted end-to-end when reopened', async ({ page }) => {
  ...
  await page.locator(`${WF} [data-part="trigger"]`).click()
  await expect(page.locator(`${WF} [data-part="calendar-grid"] tbody tr[data-week="${iso}"]`)).toHaveAttribute('data-selected', 'true')
})
...
test('the Jan/Dec boundary week carries the ISO week-year, not the visible month year', async ({ page }) => {
  ...
  const rows = page.locator(`${WF} [data-part="calendar-grid"] tbody tr`)
...
test('"This week" button sets the current ISO week', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  await page.locator(`${WF} [data-part="calendar-footer-now"]`).click()
  ...
  await expect(page.locator(`${WF} [data-part="native"]`)).toHaveValue(expected)
})
...
test('"Clear" empties the native value and closes the popup', async ({ page }) => {
  ...
  await page.locator(`${WF} [data-part="calendar-footer-now"]`).click()
  await page.locator(`${WF} [data-part="trigger"]`).click()
  await page.locator(`${WF} [data-part="calendar-footer-clear"]`).click()
  ...
  await expect(page.locator(`${WF} [data-part="popup"]`)).toHaveCount(0)
  await expect(page.locator(`${WF} [data-part="trigger"]`)).toBeFocused()
  await expect(page.locator(`${WF} [data-part="native"]`)).toHaveValue('')
})

// ── Disabled ────────────────────────────────────────────────────────────────────

test('disabled field trigger is disabled', async ({ page }) => {
  const disabledTrigger = page.locator(`.WeekField[data-disabled="true"] [data-part="trigger"]`).first()
...
test('Tab past the last footer button keeps focus inside the popup', async ({ page }) => {
  ...
  await page.locator(`${WF} [data-part="calendar-footer-now"]`).focus()
  await page.keyboard.press('Tab')
  const inside = await page.locator(`${WF} [data-part="popup"]`).evaluate((el) =>
...
test('Shift+Tab from the first tab stop keeps focus inside the popup (wraps)', async ({ page }) => {
  await page.locator(`${WF} [data-part="trigger"]`).click()
  await expect(page.locator(`${WF} [data-part="popup"]`)).toBeVisible()
  await page.locator(`${WF} [data-part="popup"] [data-part="prev-month"]`).focus()
...
test('the segment group keeps a tab stop after focus leaves it', async ({ page }) => {
  const segs = page.locator(`${WF} [data-part="segment"][tabindex]`)
...
test('Shift+Tab returns into the segment that was being edited', async ({ page }) => {
  ...
  await expect(page.locator(`${WF} [data-part="segment"][data-segment="${editing}"]`)).toBeFocused()
})
...
test('de-DE renders German weekday names, not English ones', async ({ page }) => {
  ...
  await page.locator(`${WF} [data-part="trigger"]`).first().click()
...
test('Clear is disabled while there is nothing to clear', async ({ page }) => {
  ...
  await page.locator(`${WF} [data-part="trigger"]`).first().click()
  await expect(page.locator(`${WF} [data-part="calendar-footer-clear"]`)).toBeDisabled()

  await page.locator(`${WF} [data-part="calendar-footer-now"]`).click()
  if (!(await page.locator(`${WF} [role="dialog"]`).isVisible())) {
    await page.locator(`${WF} [data-part="trigger"]`).first().click()
  }
  await expect(page.locator(`${WF} [data-part="calendar-footer-clear"]`)).toBeEnabled()
})
...
test('a mouse-opened popup takes focus, so Escape can close it', async ({ page }) => {
  ...
  const inside = await page.locator(`${WF} [role="dialog"]`).evaluate((el) =>
    // `getRootNode()` not `document`: inside an open shadow root
    // document.activeElement is the HOST, so contains() answers false.
    el.contains(el.getRootNode().activeElement))
